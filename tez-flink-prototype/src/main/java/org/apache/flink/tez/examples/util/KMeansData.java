...
import org.apache.flink.tez.examples.KMeansStepWithCross;
...
public class KMeansData {
    ...
    public static DataSet<KMeansStepWithCross.Centroid> getDefaultCentroidDataSet(ExecutionEnvironment env) {
        List<KMeansStepWithCross.Centroid> centroidList = new LinkedList<KMeansStepWithCross.Centroid>();
        for (Object[] centroid : CENTROIDS) {
            centroidList.add(
                    new KMeansStepWithCross.Centroid((Integer) centroid[0], (Double) centroid[1], (Double) centroid[2]));
        }
        return env.fromCollection(centroidList);
    }

    public static DataSet<KMeansStepWithCross.Point> getDefaultPointDataSet(ExecutionEnvironment env) {
        List<KMeansStepWithCross.Point> pointList = new LinkedList<KMeansStepWithCross.Point>();
        for (Object[] point : POINTS) {
            pointList.add(new KMeansStepWithCross.Point((Double) point[0], (Double) point[1]));
        }
